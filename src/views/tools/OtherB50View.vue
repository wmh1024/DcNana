...
const fetchAll = async () => {
  try {
    const urls = [];
    for (let i = 1; i < 8; i++) {
      urls.push(baseURL + `/Dance/api/User/GetMyRankNew?musicIndex=${i}&userId=${userId.value}`);
    }
...
const fetchPic = async () => {
  try {
    ...
    urls = [];
    for (let i = 0; i < musicListFmt.value.length; i++) {
      urls.push(baseURL + `/Dance/api/User/GetMusicRankingNew?musicIndex=0&page=1&pagesize=15&machineRank=false&keyword=${musicListFmt.value[i]?.MusicID}&isStrict=false`);
    }
...
const onSearch = (async () => {
  ...
  let res = await getInfo(userId.value)
  info.value = res.data
  res = await getMakerInfo()
  user.value = res.data
  await fetchAll()
  await fetchPic()
  closeToast()
...
<template>
  <div id="App">
    <van-search
        ...
        @search="onSearch"
    />
    <div v-if="info?.UserID" class="container">
      <van-watermark
          :content="user?.UserID"
          :full-page="true"
          :gap-x="30"
          ...
          opacity="0.5"
      />
      <h1 class="header">舞小奈の好友 B50</h1>
      <h6 style="text-align: center;margin-bottom: 20px;">
        TA 的舞立方信息
      </h6>
      <div class="row">
        <div class="col ctn">
          <img :src="info?.HeadimgURL?.replace('/132/', '/0/')" alt="Logo" class="img-thumbnail">
        </div>
        <div class="col-7">
          <div class="card" style="width: 100%;">
            <ul class="list-group list-group-flush">
              <li class="list-group-item"><strong>{{ info?.UserName }}
                <span class="badge bg-warning text-dark">{{ info?.LvRatio }}</span>
              </strong></li>
              <li class="list-group-item"><strong>ID: {{ info?.UserID }}
              </strong></li>
            ...
            >
              ...
              <template #price>
                <h6>{{ item?.PlayerPercent / 100 }}%
                  <van-tag v-if="item?.PlayerPercent === 10000" size="medium" style="margin-left: 3px;" type="warning">
                    满分
                  </van-tag>
